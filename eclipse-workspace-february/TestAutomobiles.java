...
public class TestAutomobiles {
	...
	public static void main(String[] args) {
		...
		if(isValidValues(one)) {
			System.out.println("ID number: " + one.getId() + "\nYear: " + one.getYear() +
					" \nColor: " + one.getColor() + "\nMake: " + one.getMake() 
					+ "\nModel: " + one.getModel());
		}
		else {
			System.out.println("The " + one.getColor() +" "+ one.getMake()
			+" "+ one.getModel() + " does not have valid inputs");
		}
		
		
	}

	private static boolean isValidValues(Automobile car) {
		if(car.getId() == 0 || car.getYear() == 0 || car.getMph() == 0) {
			return false;
		}
		return true;
	}
